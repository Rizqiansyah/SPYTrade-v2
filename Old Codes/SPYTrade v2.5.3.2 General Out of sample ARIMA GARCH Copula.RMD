...
p <- animate(p, 
        ...
        duration = 60, 
        end_pause = 3)
anim_save("gif/SPYTrade v2.5.3.2 - Density of price throughout verification period- US UK DE AU JP.gif", p)
p
```

```{r}
p2 <- ggplot(df_VaR_performance) +
  geom_histogram(aes(x = probability, fill = date)) +
  geom_vline(xintercept = 0.05)+
  labs(title = 'Date: {frame_time}')+
  transition_time(date)
p2 <- animate(p2, 
        ...
        duration = 60, 
        end_pause = 3)
anim_save("gif/SPYTrade v2.5.3.2 - Histogram of price throughout verification period- US UK DE AU JP.gif", p2)
p2
```
